    ...
    for token, p in list(reply_pending.items()):
        ...
        if step == "button_url":
            ...
            bot.reply_to(
                ...
                "ويمكنك وضع الإيموجي العادي داخل نص الزر، ويدعم كل الإيموجي."
            )
            return True
...
def admin_panel_markup():
    ...
    markup.row(
        button("💬 الردود", callback_data="admin:replies", style="primary", icon_custom_emoji_id=CE_REPLY_BUTTON),
        button("📝 السجل", callback_data="admin:actions", style="primary", icon_custom_emoji_id=CE_COMMANDS)
    )
    markup.row(
        button("📥 تصدير الأعضاء", callback_data="admin:export", style="success", icon_custom_emoji_id=CE_MEMBER),
        button("📤 استرجاع الأعضاء", callback_data="admin:restore", style="primary", icon_custom_emoji_id=CE_MEMBER)
    )
    markup.row(
        button("🔐 الاشتراك الإجباري", callback_data="admin:force_channels", style="danger", icon_custom_emoji_id=CE_FORCE_SUB),
        button("⚙️ التحكم", callback_data="admin:control", style="primary", icon_custom_emoji_id=CE_ADMIN)
    )
    markup.row(
        button("🔄 تحديث", callback_data="admin:refresh", style="primary", icon_custom_emoji_id=CE_REPLY_BUTTON),
        button("❌ إغلاق", callback_data="admin:close", style="danger", icon_custom_emoji_id=CE_ERROR)
    )
...
def admin_control_markup():
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.row(
        button("المجموعات", callback_data="admin:groups", style="primary", icon_custom_emoji_id=CE_MEMBER),
        button("الردود", callback_data="admin:replies", style="primary", icon_custom_emoji_id=CE_REPLY_BUTTON)
    )
    markup.row(
        button("الاشتراك الإجباري", callback_data="admin:force_channels", style="danger", icon_custom_emoji_id=CE_FORCE_SUB),
...
def restore_members_file(message):
    ...
    try:
        ...
        restored = 0
        for g in groups:
            if not isinstance(g, dict) or "chat_id" not in g:
                continue
            cols = ["chat_id","title","welcome","links","photos","videos","documents","stickers","audio","animations","spam","flood","repeat_messages","new_member_protection","max_warnings"]
            vals = [g.get(c, 0 if c not in ("title",) else "") for c in cols]
            placeholders = ",".join("?" for _ in cols)
            cursor.execute(f"INSERT OR IGNORE INTO groups({','.join(cols)}) VALUES({placeholders})", vals)
        for m in members:
            if not isinstance(m, dict) or "chat_id" not in m or "user_id" not in m:
                continue
            cursor.execute("""
                INSERT INTO group_users(chat_id,user_id,first_name,last_name,username,messages,warnings,joined_at,last_seen)
                VALUES(?,?,?,?,?,?,?,?,?)
                ON CONFLICT(chat_id,user_id) DO UPDATE SET
                    first_name=excluded.first_name,last_name=excluded.last_name,username=excluded.username,
                    messages=excluded.messages,warnings=excluded.warnings,joined_at=excluded.joined_at,last_seen=excluded.last_seen
            """, (m.get("chat_id"),m.get("user_id"),m.get("first_name", ""),m.get("last_name", ""),m.get("username", ""),m.get("messages",0),m.get("warnings",0),m.get("joined_at",0),m.get("last_seen",0)))
            restored += 1
        for fc in payload.get("force_sub_channels", []):
            if not isinstance(fc, dict) or "chat_id" not in fc:
                continue
            cursor.execute("""
            ...
            """, (fc.get("chat_id"),fc.get("username",""),fc.get("title",""),fc.get("url",""),fc.get("button_text","Update MaX"),fc.get("emoji_id",CE_FORCE_SUB),fc.get("enabled",1)))
        db.commit()
        bot.reply_to(message, f"تم استرجاع <b>{restored}</b> عضو بنجاح.")
    except Exception as e:
        print("[Restore Members Error]", e)
        bot.reply_to(message, "تعذر استرجاع الملف. تأكد أنه ملف النسخة الاحتياطية الصحيح.")
    return True
...
def force_sub_markup(user_id, channels=None):
    ...
    for row in channels:
        ...
        label = row["button_text"] or "Update MaX"
        style = "success" if subscribed else "danger"
        markup.add(button(
            label,
            callback_data=f"force_sub_check:{row['id']}",
            style=style,
            icon_custom_emoji_id=row["emoji_id"] or CE_FORCE_SUB
        ))
    return markup
...
def bot_chat_membership_handler(message):
    try:
        ...
        ensure_group(message.chat)
        print(
            f"[Group Tracking] {message.chat.id} - {message.chat.title or ''}"
        )
    except Exception as e:
...
def callbacks(call):
    ...
    try:
        ...
        if call.data.startswith("force_sub_check:"):
            try:
                ...
                if user_subscribed_to_channel(uid, row):
                    ...
                    bot.answer_callback_query(call.id, "اشترك في القناة أولًا، ثم اضغط الزر مرة أخرى.", url=row["url"] or None)
                return
            except Exception as e:
        ...
        if call.data.startswith("replybtn_more_yes:"):
            ...
            bot.send_message(
                chat_id,
                "🔘 أرسل اسم الزر التالي.\nيمكنك وضع أي إيموجي عادي داخله."
            )
            return
