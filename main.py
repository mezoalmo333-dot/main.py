...
def _youtube_runtime_options(yt_dlp):
    ...
    # أولوية الكوكيز المضمنة في المكان المخصص داخل main.py.
    cookie_file = _get_embedded_youtube_cookie_file()

    # دعم اختياري للملف الخارجي أيضًا.
    if not cookie_file:
...
def setup_default_force_channel():
    ...
    try:
        ...
        cursor.execute("DELETE FROM force_sub_channels WHERE username=? OR url=?", ("@LeaDeR_E", "https://t.me/Ssource_MaX"))
        db.commit()
