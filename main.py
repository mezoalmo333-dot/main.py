...
YOUTUBE_COOKIES = """
# الصق محتوى cookies.txt هنا فقط إذا كان main.py خاصًا بك ولا تشاركه.
# مثال:
...
def search_youtube_songs(query, limit=8):
    ...
    # البحث العام لا يحتاج تسجيل الدخول؛ android_vr لا يحتاج PO Token حاليًا.
    client_sets = [["android_vr"], ["tv"], ["web_embedded"], None]
...
def download_youtube_song(query):
    ...
    try:
        ...
        if not is_direct_url:
            for clients in (["android_vr"], ["tv"], ["web_embedded"], None):
                try:
                    opts = dict(base)
