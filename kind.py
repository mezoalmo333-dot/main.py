...
WELCOME_TEXT = (
    "Welcome to MaX VIP\n\n"
    "اشترك بـ 300 ⭐ شهريًا لفتح جميع المميزات.\n"
...
def subscription_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                colored_button("اشترك الآن — 300", callback_data="buy_subscription", style="success", emoji_id=EMOJI_SUBSCRIBE),
...
async def buy_subscription(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        await context.bot.send_invoice(
            ...
            payload=payload,
            currency="XTR",
            prices=[LabeledPrice(label="اشتراك شهري", amount=SUBSCRIPTION_STARS)],
        )
    except Exception as exc:
...
async def successful_payment_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if payment.total_amount != SUBSCRIPTION_STARS:
